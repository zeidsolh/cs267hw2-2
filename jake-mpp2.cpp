...
void simulate_one_step(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    ...
    // Compute forces (just for the particles in our band)
        ...
        for (int i = 0; i < particles.size(); i += 1) {
            int cell_x = particles[i].x / (size / grid_size);
            int cell_y = particles[i].y / (size / grid_size);
            grid[cell_y * grid_size + cell_x].particles.push_back(i);
        }
        for (int i = 0; i < lower_particles.size(); i += 1) {
            int cell_x = lower_particles[i].x / (size / grid_size);
            int cell_y = lower_particles[i].y / (size / grid_size);
            grid[cell_y * grid_size + cell_x].particles.push_back(i + particles.size());
        }
        for (int i = 0; i < upper_particles.size(); i += 1) {
            int cell_x = upper_particles[i].x / (size / grid_size);
            int cell_y = upper_particles[i].y / (size / grid_size);
            grid[cell_y * grid_size + cell_x].particles.push_back(i + particles.size() + lower_particles.size());
        }
        ...
        for (int cell_id = 0; cell_id < grid.size(); cell_id += 1) {
            ...
            for (int p_id : grid[cell_id].particles) {
                ...
                if (p_id >= particles.size()) { // Only compute forces on particles in our band
                    continue;
                }
                    ...
                    if (neighbor_row >= 0 && neighbor_row < grid_size && neighbor_col >= 0 && neighbor_col < grid_size) {
                        ...
                        for (int neighbor_p_id : grid[neighbor_cell_id].particles) {
                            if ((cell_id != neighbor_cell_id) || (p_id < neighbor_p_id)) {
                                ...
                                } else if ((neighbor_p_id >= particles.size()) && neighbor_p_id < (particles.size() + lower_particles.size())) {
                                    apply_force(particles[p_id], lower_particles[neighbor_p_id - particles.size()]);
                                } else {
                                    apply_force(particles[p_id], upper_particles[neighbor_p_id - particles.size() - lower_particles.size()]);
                                }
                            }
                        }
                    }
